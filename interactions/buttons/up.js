...
module.exports = {
    ...
    run: async (client, interaction) => {
        ...
        let slider = SliderStore.get(interaction.customId.replace("up", ""));
        ...
        .then(async res => {
            if(res.length == 0) {
                client.database.request("INSERT INTO votes (upvote, user_id, image_id) VALUES (1, ?, ?);", [interaction.user.id, actualRes.id])
                .then(async () => {
                    await interaction.reply({ content: text.get("aiImageSlider", "voteUpSuccess"), ephemeral: true });
                    actualRes.upvote += 1;
                    slider.reload();
                })
                ...
                if(res[0].upvote == 1) {
                    await interaction.reply({ content: text.get("aiImageSlider", "voteUpAlready"), ephemeral: true });
                    slider.reload()
